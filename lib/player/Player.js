...
export class EluvioPlayer {
  ...
  async __InitializeHLS({playoutUrl, authorizationToken, drm, drms, multiviewOptions}) {
    ...
    if(
      ...
      if(this.playerOptions.defaultSubtitleTrack) {
        hlsPlayer.on(
          this.HLS.Events.SUBTITLE_TRACKS_UPDATED,
          () => {
            const langCode = this.playerOptions.defaultSubtitleTrack;
            const textTrack = (this.controls.GetTextTracks().options || []).find(
              track => track.language === langCode || (track.language && track.language.startsWith(langCode + "-"))
            );

            if(textTrack && textTrack.index >= 0) {
              this.controls.SetTextTrack(textTrack.index);
            }
          }
        );
      }
